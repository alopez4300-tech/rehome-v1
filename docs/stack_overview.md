...
* *(Phase 2)* `project_summaries` (id, project_id, type enum('daily','weekly','milestone','meeting'), content markdown, generated_at)
* *(Optional)* `workspaces.ai_model`, `workspaces.ai_budget_cents`
...
* Environment/config:
  ...
  * `.env` essentials: `APP_KEY`, `APP_URL`, DB*, REDIS*, MAIL*, QUEUE*, CACHE_DRIVER, FILESYSTEM_DISK, **BROADCAST_***, **AI_***, `SANCTUM_STATEFUL_DOMAINS`, `CORS_*`
